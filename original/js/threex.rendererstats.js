...
*/
THREEx.RendererStats	= function (fileSize){
	...
	msDiv.style.cssText = 'padding:0 0 3px 3px;text-align:left;background-color:#002;';
	...
	for(var i = 0; i < nLines; i++){
		...
		msTexts[i].style.cssText = 'color:#0ff;background-color:#122;font-family:Helvetica,Arial,sans-serif;font-size:9px;font-weight:bold;line-height:15px';
	...
	return {
		...
		update: function(webGLRenderer, fileSize){
			if (fileSize) {
				size = fileSize;
			}
			...
			msTexts[i++].textContent = "File Size: "	+ size;
			if (webGLRenderer){
				// sanity check
